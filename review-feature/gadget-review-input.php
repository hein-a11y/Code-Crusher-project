...
$sql = $pdo->prepare("SELECT gadget.gadget_name FROM gg_gadget AS gadget WHERE gadget.gadget_id = ? LIMIT 1");
$sql ->execute([$currentGadgetId]);
$currentProduct = $sql->fetchAll();
$currentUserName = $_SESSION['customer']['firstname'] . " " . $_SESSION['customer']['lastname'];
$currentProductName = $currentProduct[0]['gadget_name'];
