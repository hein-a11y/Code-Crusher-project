...
function validateForm() {
    ...
    if (!katakanaRegex.test(firstname_kana.value)){
        isValid = false;
        showError(firstname_kana,"first_katakana_error","フリガナ (firstname_kana) には、全角**カタカナ**のみを使用してください。");
    }

    const lastname_kana = document.getElementById("lastname_kana");
    if (!katakanaRegex.test(lastname_kana.value)){
        isValid = false;
        showError(lastname_kana,"last_katakana_error","フリガナ (lastname_kana) には、全角**カタカナ**のみを使用してください。");
    }
 ...
 */
function showError(field,error_id = 'none', message) {
    let mistakeField = document.getElementById(`${error_id}`);
    ...
    if (field.type === "checkbox") {
        ...
        if(message != undefined){
            mistakeField.textContent = message;
        }
              
    }
    console.log(message);
    console.log(mistakeField);
}
